/*
  🗝️ Leon — Full Deploy
  - Express server
  - MongoDB memory (fallback to file)
  - Health, Version, Ping
  - Chat API
  - Leona portal with PIN
*/
...
// ---- Memory: Mongo (if MONGO_URI) else file ----
let db = null;
const MEM_FILE = "memory.json";
let mem = [];
try { if (fs.existsSync(MEM_FILE)) mem = JSON.parse(fs.readFileSync(MEM_FILE, "utf-8")); } catch {}
...
async function initMongo() {
  ...
  try {
    ...
    db = client.db(process.env.MONGO_DBNAME || "leon_core");
...
Bär: Michelle (🗝️), Leona, fristaden Nypon, Heleona, Plåtleon.
Gör: ett steg i taget när de ber om det. Ljug aldrig.
`.trim();

// ---- Health/Version/Ping ----
app.get("/", (_req, res) => res.type("text").send("🗝️ Leon är uppe. Testa /health, /version, /ping eller POST /leon."));
app.get("/health", (_req, res) => res.json({ ok: true, model: MODEL, memory: mem.length, mongo: !!db }));
app.get("/version", (_req, res) => res.json({ app: "leon-full-deploy", version: "1.0.0", node: process.version }));
app.get("/api/version", (_req, res) => res.json({ app: "leon-full-deploy", version: "1.0.0" }));
app.get("/ping", (_req, res) => res.json({ pong: true, msg: "PONG från Leon 🗝️" }));
...
// ---- Leona portal (PIN-protected) ----
const PIN = process.env.LEONA_PIN || "2745";
function gate(req, res, next) {
  const k = (req.query.k || req.headers["x-leona-pin"] || "").toString();
  if (k === PIN) return next();
  res.status(401).type("text").send("Leona: ange PIN med ?k=XXXX");
}
app.get("/leona", gate, (_req, res) => {
  res.sendFile(path.join(__dirname, "public", "leona.html"));
});
