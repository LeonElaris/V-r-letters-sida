// 🗝️ Plåtleon — server.js (PIN-skyddad Leona-portal + Mongo fallback)
...
// ---- Health/Version ----
app.get("/", (_req, res) => res.type("text").send("🗝️ Plåtleon är uppe. Testa /health, /version eller /leona?k=PIN."));
app.get("/health", (_req, res) => res.json({ ok: true, model: MODEL, memory: mem.length, mongo: !!db }));
app.get("/version", (_req, res) => res.json({ app: "platleon", version: "1.3.0", node: process.version }));
app.get("/api/version", (_req, res) => res.json({ app: "platleon", version: "1.3.0" }));

// ---- Enkel PIN-grind för Leona-sidan ----
